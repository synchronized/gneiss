...
result runtime_scene_inspection::capture(std::span<const runtime_scene_source_node> nodes,
  ...
  try {
    ...
    for (auto iterator = identities.begin(); iterator != identities.end();) {
      if (uuids.contains(iterator->first)) {
        ++iterator;
        continue;
      }
    ...
    for (const auto& node : nodes) {
      if (identities.contains(node.uuid)) {
        continue;
      }
      ...
      identities.emplace(node.uuid, ipc_runtime_object_id{value, generations.at(value)});
    }

    std::map<std::uint64_t, ipc_runtime_object_id> native_identities;
    for (const auto& node : nodes) {
      native_identities.emplace(node.native_node, identities.at(node.uuid));
    }
    ...
    for (const auto& source : nodes) {
      ...
      runtime_scene_snapshot_node node{.id = identities.at(source.uuid),
                                       .parent = parent,
                                       .uuid = source.uuid,
                                       .name = source.name,
